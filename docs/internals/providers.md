...
Cursor, Grok, and OpenCode implementations remain in the source tree for upstream parity, but this
custom build does not register them as built-in drivers.
...
- Codex, Claude, Cursor, Grok, and Kiro send images as native image inputs and skip generic files. For
