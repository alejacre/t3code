...
| Kiro     | [Kiro CLI](https://kiro.dev/cli/)                     | `kiro-cli`     | Interactive Kiro sign-in |

Codex and Claude are on by default. Kiro is off by default; turn it on in **Settings** → the
provider's card when you want to use it.

Kiro runs through its ACP interface. T3 Code discovers the model catalog from the installed CLI;
the optional **Agent engine** and **Default agent** fields are advanced settings for Kiro
installations that expose multiple engines or named agents.
