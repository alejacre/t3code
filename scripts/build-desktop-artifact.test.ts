...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.40-beta.2"), "T3 Custom Beta");
    assert.equal(resolveDesktopProductName("0.0.17"), "T3 Custom");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "T3 Custom (Nightly)");
  });
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "T3 Custom 1.2.3 Installer",
      ...
      assert.deepStrictEqual((linux.linux as Record<string, unknown>).protocols, [
        { name: "T3 Custom", schemes: ["t3code", "t3code-dev"] },
      ]);
