...
export function buildKiroAcpSpawnInput(
  ...
  // Both flags are optional overrides. The default spawn is a plain
  // `kiro-cli acp`, so T3 Code does not depend on CLI builds that ship the
  // `--agent-engine` flag unless the user opted into a specific engine.
  const agentEngine = kiroSettings.agentEngine.trim();
  const agent = kiroSettings.agent.trim();
  return {
    command: kiroSettings.binaryPath || "kiro-cli",
    args: [
      "acp",
      ...(agentEngine.length > 0 ? ["--agent-engine", agentEngine] : []),
      ...(agent.length > 0 ? ["--agent", agent] : []),
    ],
    cwd,
