...
describe("Kiro model catalog", () => {
  ...
  it("tolerates extra catalog fields emitted by the real CLI", () => {
    // Shape captured from `kiro-cli chat --list-models --format json` (2.20.0).
    const models = KIRO_PROVIDER_TESTING.parseKiroModels(
      JSON.stringify({
        models: [
          {
    ...
    expect(models?.map((model) => model.slug)).toEqual(["auto"]);
  });
