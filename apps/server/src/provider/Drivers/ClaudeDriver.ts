...
export function resolveClaudeMaintenanceCapabilities(
  ...
  const commandPaths = context ? [context.resolvedCommandPath, context.realCommandPath] : [];
  if (context && commandPaths.some(isClaudeToolboxCommandPath)) {
  ...
  return PUBLIC_UPDATE.resolve(context);
}
