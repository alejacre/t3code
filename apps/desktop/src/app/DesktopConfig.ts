...
export const DesktopConfig = Config.all({
  ...
  // T3 Custom: absolute path for Electron's userData (Chromium profile and the
  // single-instance lock). Lets a side-by-side test build run next to the
  // installed one; pair it with T3CODE_HOME so the T3 state is separate too.
  userDataDirOverride: trimmedString("T3CODE_DESKTOP_USER_DATA_DIR"),
